...
jQuery(document).ready(function() {
	...
	if (jQuery().appear) {
		...
		jQuery('.to_animate').filter(':appeared').each(function(index){
			...
			var animationDelay = !self.data('delay') ? 300 : self.data('delay');
			setTimeout(function(){
	...
	jQuery('.isotope-item, .horizontal_slider_introimg, .portfolio_item_image').hover(
	 	function() {
			jQuery( this ).find('.portfolio_links a').stop().animate({ opacity: 1}, 50, 'easeOutExpo');
		}, function() {
			jQuery( this ).find('.portfolio_links a').stop().animate({ opacity: 0}, 50, 'easeOutExpo');
		}
	);
